...
asses_fit_cols <- text_spec(c("Evidence", "Suspect", "5th", "50th", "95th"), format = "latex", bold = TRUE)
...
probs %>% 
  ...
  kable(booktabs = TRUE, col.names = asses_fit_cols, format = "latex", 
...
```{r monte-fig, fig.width = 10, fig.height = 7, fig.align = "center", fig.cap = "Posterior probability distributions for each suspect's guilt given each subsequent piece of evidence produced", dpi=720, out.width = "7in", out.height = "4in"}
...
After assessing the accuracy of the simulations, the monte carlo analysis was completed by calculating the posterior probabilities for each suspect for each of the five pieces of evidence presented (Figure \ref{fig:monte-fig}).  With each subsequent piece of evidence presented, it became more and more likely that X had committed the crime. In addition, the uncertainty in the posterior probabilities decreased for all suspects with each subsequent piece of evidence. For example, the prior probabilty that X was guilty had a 95% confidence interval of (`r monte_unnested %>% filter(str_detect(key, "Prior"), str_detect(person, "X")) %>% pull(prob) %>% quantile(0.025) %>% round(3)` - `monte_unnested %>% filter(str_detect(key, "Prior"), str_detect(person, "X")) %>% pull(prob) %>% quantile(0.975) %>% round(3)`) and the final posterior probabilty that X was guilty, Prob[call to X friend|suspect], had a 95% confidence interval of (`r monte_unnested %>% filter(str_detect(key, "call to X"), str_detect(person, "X")) %>% pull(prob) %>% quantile(0.025) %>% round(3)` - `r monte_unnested %>% filter(str_detect(key, "call to X"), str_detect(person, "X")) %>% pull(prob) %>% quantile(0.975) %>% round(3)`). 


```{r uncertainty-summary}
summary_table_cols <- text_spec(c("Probability", "Suspect", "Median", "Std. Dev.", "95% CI"), format = "latex", bold = TRUE)
...
monte_unnested %>% 
  ...
  arrange(key, person) %>% 
  kable(booktaps = TRUE, escape = FALSE, format = "latex", 
        col.names = summary_table_cols, longtable = TRUE,
